...
def pad_or_trim_to_shape(x, shape):
...
def clip_boxes(boxes, image_height,
               image_width):
...
def get_vehicle_xy(image_height, image_width,
                   x_range,
                   y_range):
...
def frame_to_image_coord(frame_xy, vehicle_xy,
                         one_over_resolution):
  ...
  image_xy[Ellipsis, 0] += vehicle_xy[0]
  image_xy[Ellipsis, 1] = vehicle_xy[1] - 1 - image_xy[Ellipsis, 1]
  return image_xy


def image_to_frame_coord(image_xy, vehicle_xy,
                         resolution):
  ...
  frame_xy[Ellipsis, 0] = (frame_xy[Ellipsis, 0] - vehicle_xy[0]) * resolution
  frame_xy[Ellipsis, 1] = (vehicle_xy[1] - 1 - frame_xy[Ellipsis, 1]) * resolution
  return frame_xy


def frame_to_image_boxes(frame_boxes, vehicle_xy,
                         one_over_resolution):
  ...
  Returns:
    image_boxes: A [N, 4] array or tensor, [ymin, xmin, ymax, xmax] in image
      coordinate.
  """
  center_x = frame_boxes[Ellipsis, 0]
  center_y = frame_boxes[Ellipsis, 1]
  box_length = frame_boxes[Ellipsis, 2]
  box_width = frame_boxes[Ellipsis, 3]
...
def image_to_frame_boxes(image_boxes, vehicle_xy,
                         resolution):
  ...
  Returns:
    frame_boxes: A [N, 4] array or tensor, [center_x, center_y, length, width]
      in frame coordinate.
  """
  ymin = image_boxes[Ellipsis, 0]
  xmin = image_boxes[Ellipsis, 1]
  ymax = image_boxes[Ellipsis, 2]
  xmax = image_boxes[Ellipsis, 3]
...
def generate_anchors(min_level, max_level, image_size,
                     anchor_sizes):
