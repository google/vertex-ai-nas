...
# ==============================================================================
"""Basic operations for image augmentation."""
...
IntOrTensor = Union[int, tf.Tensor]
...
def _cutout_base(image,
                 ...
                 cutout_image = True,
                 cutout_mask = False):
  """Apply cutout to an image and segmentation mask.

  Cutout reference: https://arxiv.org/abs/1708.04552

  This operation applies a (2*pad_size x 2*pad_size) mask of zeros to
  a random location. The cutout regions are filled with the `replace`
  and `mask_replace` values.
  ...
  Args:
    ...
    segmentation_mask: A Tensor of type uint8.
    mask_replace: What pixel value to fill in the segmentation_mask in the area
      that has the cutout mask applied to it.
    cutout_image: Whether or not to apply cutout to the image.
    cutout_mask: Whether or not to apply cutout to the segmentation_mask.

  Returns:
    The resulting image and segmentation_mask tensors after applying cutout.
  """
  ...
  if cutout_image:
    image = tf.where(
        ...
        image)
  if segmentation_mask is not None and cutout_mask:
    equal = tf.reshape(
        tf.equal(mask[:, :, 0:1], 0), tf.shape(segmentation_mask))
    segmentation_mask = tf.where(equal,
                                 tf.ones_like(segmentation_mask) * mask_replace,
                                 segmentation_mask)
...
def cutout(image,
  ...
  This operation applies a (2*pad_size x 2*pad_size) mask of zeros to
  a random location. The cutout regions are filled with the `replace` value.
  ...
  Returns:
    The resulting image tensor after applying cutout.
  """
  return _cutout_base(image=image,
                      pad_size=pad_size,
                      replace=replace,
                      segmentation_mask=None,
                      cutout_image=True,
...
def cutout_image_and_mask(image,
                          segmentation_mask,
                          pad_size,
                          replace = 0,
                          mask_replace = 255,
...
def rotate_with_mask(image,
  ...
  Returns:
    The rotated version of image and segmentation_mask.
  """
  # Convert from degrees to radians.
  degrees_to_radians = math.pi / 180.0
  radians = degrees * degrees_to_radians
  ...
  # it negatively half the time, but that's done on 'degrees' outside
  # of the function.
  orig_mask_shape = tf.shape(segmentation_mask)
  image = contrib_image.rotate(
      wrap_image_and_mask(image, segmentation_mask), radians)
  image, segmentation_mask = unwrap_image_and_mask(image,
                                                   replace=replace,
                                                   mask_replace=mask_replace)
  return image, tf.reshape(segmentation_mask, orig_mask_shape)


def random_shift_bbox(
    image,
    bbox,
    pixel_scaling,
    replace,
...
def translate_x_only_bboxes(image,
                            bboxes,
                            prob,
                            pixels,
                            replace):
...
def translate_y_only_bboxes(image,
                            bboxes,
                            prob,
                            pixels,
                            replace):
...
def solarize_only_bboxes(image,
                         bboxes,
                         prob,
                         threshold):
...
def equalize_only_bboxes(image,
                         bboxes,
                         prob):
...
def translate_x(image,
  ...
  Args:
    ...
    pixels: Number of pixels to shift.
    replace: Pixel value to replace shifted region.
...
def translate_x_with_mask(image,
                          segmentation_mask,
                          pixels,
                          replace,
                          mask_replace):
  ...
  Args:
    image: An image Tensor.
    segmentation_mask: A segmentation mask Tensor.
    pixels: Number of pixels to shift.
    replace: Pixel value to replace the image shifted region.
    mask_replace: Pixel value to replace the segmentation_mask shifted region.

  Returns:
    The shifted image and segmentation_mask.
  """
  orig_mask_shape = tf.shape(segmentation_mask)
  image = contrib_image.translate(
      wrap_image_and_mask(image, segmentation_mask), [-pixels, 0])
  image, segmentation_mask = unwrap_image_and_mask(image,
                                                   replace=replace,
                                                   mask_replace=mask_replace)
  return image, tf.reshape(segmentation_mask, orig_mask_shape)
...
def translate_y_with_mask(image,
                          segmentation_mask,
                          pixels,
                          replace,
                          mask_replace):
  ...
  orig_mask_shape = tf.shape(segmentation_mask)
  image = contrib_image.translate(
      wrap_image_and_mask(image, segmentation_mask), [0, -pixels])
  image, segmentation_mask = unwrap_image_and_mask(image,
                                                   replace=replace,
                                                   mask_replace=mask_replace)
  return image, tf.reshape(segmentation_mask, orig_mask_shape)
...
def shear_x_with_mask(image,
  ...
  orig_mask_shape = tf.shape(segmentation_mask)
  image = contrib_image.transform(
      wrap_image_and_mask(image, segmentation_mask),
      [1., level, 0., 0., 1., 0., 0., 0.])
  image, segmentation_mask = unwrap_image_and_mask(image,
                                                   replace=replace,
                                                   mask_replace=mask_replace)
...
def wrap_image_and_mask(image,
                        segmentation_mask):
  """Concats image and mask and an extra channel set to all 1s."""
  shape = tf.shape(image)
  # Ensure segmentation mask has 3 dimensions.
  seg_shape = tf.concat([shape[:2], [1]], axis=0)
  segmentation_mask = tf.reshape(segmentation_mask, seg_shape)
  extended_channel = tf.ones([shape[0], shape[1], 1], image.dtype)
  extended = tf.concat([image, segmentation_mask, extended_channel], 2)
  return extended


def unwrap_image_and_mask(image,
                          replace,
                          mask_replace):
  """Unwraps an image and segmentatin_mask produced by wrap_image_and_mask.

  Args:
    image: A 3D Tensor containing a wrapped image and segmentation_mask.
    replace: What pixel value to fill in the image in the area that has the
      cutout mask applied to it.
    mask_replace: What pixel value to fill in the segmentation_mask in the area
      that has the cutout mask applied to it.
  ...
  Returns:
    The resulting unwrapped image and segmentation_mask.
  """
  ...
  alpha_channel = flattened_image[:, 4]

  ones = tf.ones([1], image.dtype)
  replace = tf.concat([replace, mask_replace * ones, ones], 0)
  ...
  segmentation_mask = tf.slice(image, [0, 0, 3],
