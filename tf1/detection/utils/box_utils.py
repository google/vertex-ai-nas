...
def jitter_boxes(boxes, noise_scale=0.025):
  ...
  with tf.name_scope('jitter_boxes'):
    ...
    width = xmax - xmin
    height = ymax - ymin
    new_center_x = (xmin + xmax) / 2.0 + bbox_jitters[Ellipsis, 0:1] * width
    new_center_y = (ymin + ymax) / 2.0 + bbox_jitters[Ellipsis, 1:2] * height
    new_width = width * tf.exp(bbox_jitters[Ellipsis, 2:3])
    new_height = height * tf.exp(bbox_jitters[Ellipsis, 3:4])
...
def normalize_boxes(boxes, image_shape):
  ...
  with tf.name_scope('normalize_boxes'):
    ...
    ymin = boxes[Ellipsis, 0:1] / height
    xmin = boxes[Ellipsis, 1:2] / width
    ymax = boxes[Ellipsis, 2:3] / height
    xmax = boxes[Ellipsis, 3:4] / width
  ...
  if scale < 1.0:
    raise ValueError(
        'scale is {}, but outer box scale must be greater than 1.0.'.format(
            scale))
  centers_y = (boxes[Ellipsis, 0] + boxes[Ellipsis, 2]) / 2.0
  centers_x = (boxes[Ellipsis, 1] + boxes[Ellipsis, 3]) / 2.0
  box_height = (boxes[Ellipsis, 2] - boxes[Ellipsis, 0]) * scale
  box_width = (boxes[Ellipsis, 3] - boxes[Ellipsis, 1]) * scale
