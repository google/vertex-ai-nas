...
from tf1.detection.serving import classification

from tf1.detection.serving import detection
from tf1.detection.serving import segmentation
...
def export(export_dir,
  ...
  if model in _DETECTION_MODELS:
    ...
    # Skip padding for classification model since the input image size
    # (e.g., 224) can be different from `stride 2^max_level`.
    model_fn = classification.serving_model_fn_builder(
        use_tpu, output_image_info)
    serving_input_receiver_fn = functools.partial(
        classification.serving_input_fn,
        batch_size=batch_size,
        desired_image_size=image_size,
        stride=2,
        input_type=input_type,
